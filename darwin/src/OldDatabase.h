...
//#include <list>
#include <vector> //***1.85
#include <algorithm> //***!.85 - for std::sort()
#include <stdio.h>
#include <stdlib.h>
...
template <class OldDatabase_IMAGE_TYPE>
class OldDatabase
{
	public:
		...
		OldDatabase(Options *o, bool createEmptyDB); //***054 - since catagories may change

		~OldDatabase();
		
		void createEmptyOldDatabase(Options *o); //***054

		unsigned long add(DatabaseFin<OldDatabase_IMAGE_TYPE>* data); //***1.85 - return type changed

		DatabaseFin<OldDatabase_IMAGE_TYPE>* getItemAbsolute(unsigned pos); //***1.3

		DatabaseFin<OldDatabase_IMAGE_TYPE>* getItem(unsigned pos);
		DatabaseFin<OldDatabase_IMAGE_TYPE>* getItemByName(std::string name);
		...
		void DeleteFinFromList(DatabaseFin<OldDatabase_IMAGE_TYPE> *Fin); //***002DB
                
		void Delete(DatabaseFin<OldDatabase_IMAGE_TYPE> *Fin); //***002DB
		...
		{
			public:
				BoundsError() : Error("Attempt to access element out of OldDatabase bounds.")
				{ }
		};
	...
	protected:
		...
		//DatabaseFin<OldDatabase_IMAGE_TYPE>* getItem(unsigned pos, std::list<std::string>::iterator it);
		DatabaseFin<OldDatabase_IMAGE_TYPE>* getItem(unsigned pos, std::vector<std::string> *theList);
};
...
//
template <class OldDatabase_IMAGE_TYPE>
void OldDatabase<OldDatabase_IMAGE_TYPE>::convert(std::string filename)
{
}
...
//
template <class OldDatabase_IMAGE_TYPE>
OldDatabase<OldDatabase_IMAGE_TYPE>::OldDatabase(Options *o, bool createEmptyDB) 
	:
...
//
template <class OldDatabase_IMAGE_TYPE>
OldDatabase<OldDatabase_IMAGE_TYPE>::~OldDatabase()
{
	mDbFile.close();
}
...
//
template <class OldDatabase_IMAGE_TYPE>
bool OldDatabase<OldDatabase_IMAGE_TYPE>::closeStream()
{
	mDbFile.close();
...
//
template <class OldDatabase_IMAGE_TYPE>
bool OldDatabase<OldDatabase_IMAGE_TYPE>::openStream()
{
	mDbFile.open(mFilename.c_str(), ios::in | ios::out | ios::binary);
	return (! mDbFile.fail());
}
...
//
template <class OldDatabase_IMAGE_TYPE>
void OldDatabase<OldDatabase_IMAGE_TYPE>::createEmptyOldDatabase(Options *o)
{
	mFooterPos = 0;
	mDataSize = 0;
	mHeaderSize = 0;
...
//
template <class OldDatabase_IMAGE_TYPE>
//bool OldDatabase<OldDatabase_IMAGE_TYPE>::add(DatabaseFin<OldDatabase_IMAGE_TYPE> *data)
unsigned long OldDatabase<OldDatabase_IMAGE_TYPE>::add(DatabaseFin<OldDatabase_IMAGE_TYPE> *data) //***1.85
{
#ifdef DEBUG
...
//
template <class OldDatabase_IMAGE_TYPE>
unsigned OldDatabase<OldDatabase_IMAGE_TYPE>::size() const
{
...
//
template <class OldDatabase_IMAGE_TYPE>
unsigned OldDatabase<OldDatabase_IMAGE_TYPE>::sizeAbsolute() const
{
...
//
template <class OldDatabase_IMAGE_TYPE>
bool OldDatabase<OldDatabase_IMAGE_TYPE>::isEmpty() const
{
...
//
template <class OldDatabase_IMAGE_TYPE>
typename OldDatabase<OldDatabase_IMAGE_TYPE>::db_status_t OldDatabase<OldDatabase_IMAGE_TYPE>::status() const
{
	return mDBStatus;
}
...
//
template <class OldDatabase_IMAGE_TYPE>
db_sort_t OldDatabase<OldDatabase_IMAGE_TYPE>::currentSort()
{
	return mCurrentSort;
}
...
//
template <class OldDatabase_IMAGE_TYPE>
int OldDatabase<OldDatabase_IMAGE_TYPE>::getIDListPosit(std::string id)
{
	// NOTE: this list in the OldDatabase contains strings, but the strings
	// are made up of two parts ("the Id" and "the byte offset" of the fin 
	// in the OldDatabase file)
...
//
template <class OldDatabase_IMAGE_TYPE>
DatabaseFin<OldDatabase_IMAGE_TYPE>* OldDatabase<OldDatabase_IMAGE_TYPE>::getItemAbsolute(unsigned pos)
{
#ifdef DEBUG
	...
	DatabaseFin<OldDatabase_IMAGE_TYPE> *fin = new DatabaseFin<OldDatabase_IMAGE_TYPE>(mDbFile);
...
//
template <class OldDatabase_IMAGE_TYPE>
DatabaseFin<OldDatabase_IMAGE_TYPE>* OldDatabase<OldDatabase_IMAGE_TYPE>::getItem(unsigned pos)
{
#ifdef DEBUG
...
//
template <class OldDatabase_IMAGE_TYPE>
DatabaseFin<OldDatabase_IMAGE_TYPE>* OldDatabase<OldDatabase_IMAGE_TYPE>::getItemByName(std::string name)
{
#ifdef DEBUG
...
//
template <class OldDatabase_IMAGE_TYPE>
ItemInfo* OldDatabase<OldDatabase_IMAGE_TYPE>::getItemInfo(unsigned pos)
{
#ifdef DEBUG
...
//
template <class OldDatabase_IMAGE_TYPE>
void OldDatabase<OldDatabase_IMAGE_TYPE>::writeFooter(){
...
//
template <class OldDatabase_IMAGE_TYPE>
void OldDatabase<OldDatabase_IMAGE_TYPE>::DeleteFinFromList(DatabaseFin<OldDatabase_IMAGE_TYPE> *Fin)
{
...
//*******************************************************************
//
// void OldDatabase<OldDatabase_IMAGE_TYPE>::Delete(DatabaseFin<OldDatabase_IMAGE_TYPE> *Fin)
//
...
//
template <class OldDatabase_IMAGE_TYPE>
void OldDatabase<OldDatabase_IMAGE_TYPE>::sort(db_sort_t sortBy)
{
	mCurrentSort = sortBy;
}
...
//
template <class OldDatabase_IMAGE_TYPE>
//DatabaseFin<OldDatabase_IMAGE_TYPE>* OldDatabase<OldDatabase_IMAGE_TYPE>::getItem(unsigned pos, std::list<std::string>::iterator it)
DatabaseFin<OldDatabase_IMAGE_TYPE>* OldDatabase<OldDatabase_IMAGE_TYPE>::getItem(unsigned pos, std::vector<std::string> *theList)
{
	//for (unsigned i = 0; i < pos; i++)
	//	it++;
...
//
template <class OldDatabase_IMAGE_TYPE>
string OldDatabase<OldDatabase_IMAGE_TYPE>::getItemEntryFromList(db_sort_t whichList, unsigned pos)
{
	if (pos > this->size())
	       throw BoundsError();
	...
	//using OldDatabase<OldDatabase_IMAGE_TYPE>::db_sort_t;
...
//
template <class OldDatabase_IMAGE_TYPE>
int OldDatabase<OldDatabase_IMAGE_TYPE>::getItemListPosFromOffset(db_sort_t whichList, string item)
{
...
//
template <class OldDatabase_IMAGE_TYPE>
string OldDatabase<OldDatabase_IMAGE_TYPE>::getFilename()
{
	return mFilename;
}
